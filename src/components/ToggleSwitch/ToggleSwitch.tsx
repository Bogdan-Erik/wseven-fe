...
export const ToggleSwitch = ({ label, name, id, classNames, value }: ToggleSwitchProps): JSX.Element => {
  return (
    <label htmlFor={id} className="inline-flex relative items-center cursor-pointer">
      <input type="checkbox" value={value ?? ''} name={name} id={id} className="sr-only peer" defaultChecked={value === true} />
      ...
      <span className="ml-3 text-[16px] font-medium text-gray-900 dark:text-gray-300">{label}</span>
    </label>
  )
}